...
function markdown(content, discussion = 0, title = '') {
	...
	function parseTable(content) {
		...
		for(_tr of (data.match(/^(<tr norender><td>(((?!<\/td><\/tr>($|\n))[\s\S])*)<\/td><\/tr>)$/gim) || [])) {
		...
		// 셀 꾸미기
		for(_tr of (data.match(/^<tr>(((?!<\/tr>).)*)<\/tr>$/gim) || [])) {
			var tr = _tr.match(/^<tr>(((?!<\/tr>).)*)<\/tr>$/im)[1], ntr = tr;
			
			for(td of (tr.match(/<td>(((?!<\/td>).)*)<\/td>/g) || [])) {
	...
	for(esc of (data.match(/(?:\\)(.)/g) || [])) {
		const match = data.match(/(?:\\)(.)/);
		data = data.replace(esc, '<span>' + match[1] + '</span>');
	}
	
	for(link of (data.match(/\[\[(((?!\]\]).)+)\]\]/g))) {
	...
	for(cbl of (data.match(/}}}/g) || [])) {
		data = data.replace(cbl, '\n' + blocks.top());
		blocks.pop();
	}
	...
	var tochtml = '\n<div class=wiki-macro-toc id=toc>', tocarr = [];
	...
	// 사실 내가 봐도 이건 너무...
	for(heading of (data.match(/^(=\s(((?!=).)*)\s=|==\s(((?!==).)*)\s==|===\s(((?!===).)*)\s===|====\s(((?!====).)*)\s====|=====\s(((?!=====).)*)\s=====|======\s(((?!======).)*)\s======)$/gm) || [])) {
		...
		if(h6) {
			...
			data = data.replace(heading, '\n</div><h6 class=wiki-heading><a href="#toc" id="s-' + snum + '">' + snum + '.</a> ' + html.escape(title) + '</h6><div class=wiki-heading-content>\n');
			...
			data = data.replace(heading, '\n</div><h5 class=wiki-heading><a href="#toc" id="s-' + snum + '">' + snum + '.</a> ' + html.escape(title) + '</h5><div class=wiki-heading-content>\n');
			...
			data = data.replace(heading, '\n</div><h4 class=wiki-heading><a href="#toc" id="s-' + snum + '">' + snum + '.</a> ' + html.escape(title) + '</h4><div class=wiki-heading-content>\n');
			...
			data = data.replace(heading, '\n</div><h3 class=wiki-heading><a href="#toc" id="s-' + snum + '">' + snum + '.</a> ' + html.escape(title) + '</h3><div class=wiki-heading-content>\n');
			...
			data = data.replace(heading, '\n</div><h2 class=wiki-heading><a href="#toc" id="s-' + snum + '">' + snum + '.</a> ' + html.escape(title) + '</h2><div class=wiki-heading-content>\n');
			...
			data = data.replace(heading, '\n</div><h1 class=wiki-heading><a href="#toc" id="s-' + snum + '">' + snum + '.</a> ' + html.escape(title) + '</h1><div class=wiki-heading-content>\n');
...
console.log(markdown(content, 0, 'abc'));
...
http.createServer((req, res) => {
    ...
    res.end('<meta charset=utf8 /><div class=wiki-article>' + markdown(content, 0, 'abc') + '</div>');
}).listen(80, '127.4.12.0');
